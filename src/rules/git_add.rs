...
impl Rule for GitAdd {
    ...
    fn generate_corrections(&self, _command: &Command) -> Vec<Correction> {
        ...
        // Whether they typed `git add` or `git add oops`, we just suggest `git add --all` and `git add .`
        // Actually, user wants 'git add -A' or 'git add .'.

        // Priority to 'git add -A' and 'git add .'
